...
-   **Configuração**: `staticwebapp.config.json` marca `/admin` e `/admin/*` com `allowedRoles: ["admin"]`. Os usuários associados a essa role são definidos no portal do SWA (modo *Simple*).
-   **Fluxo**: A página `/login` exibe um botão que chama `/.auth/login/aad?post_login_redirect_uri=/admin`. Após o login o SWA cria o cookie `.auth` e libera `/admin` para o usuário autenticado.
-   **Sessão**: O front-end consulta `/.auth/me` para detectar sessões existentes e redirecionar automaticamente administradores já autenticados. Não há armazenamento manual de tokens ou cookies personalizados.
...
### 5. Login (`/login`)
Página responsável por iniciar o fluxo de autenticação.
-   Valida o reCAPTCHA (quando habilitado) e redireciona o usuário para `/.auth/login/aad?post_login_redirect_uri=/admin`.
-   Consulta `/.auth/me` para detectar sessões já autenticadas e envia o usuário diretamente para `/admin`.